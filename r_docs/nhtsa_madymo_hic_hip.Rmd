...
madymo_nhtsa_df$WEATHER[madymo_nhtsa_df$WEATHER == 2] <- "sleet/hail/fog"
madymo_nhtsa_df$WEATHER[madymo_nhtsa_df$WEATHER == 3] <- "snow"
madymo_nhtsa_df$WEATHER[madymo_nhtsa_df$WEATHER == 4] <- "sleet/hail/fog"
madymo_nhtsa_df$WEATHER[madymo_nhtsa_df$WEATHER == 5] <- "sleet/hail/fog"
madymo_nhtsa_df$WEATHER[madymo_nhtsa_df$WEATHER == 6] <- "sleet/hail/fog"
madymo_nhtsa_df$WEATHER[madymo_nhtsa_df$WEATHER == 7] <- "other"
madymo_nhtsa_df$WEATHER[madymo_nhtsa_df$WEATHER == 8] <- "unknown"
# establish WEATHER = 0 as reference value
madymo_nhtsa_df$WEATHER <- factor(
  madymo_nhtsa_df$WEATHER, 
  levels = c(
    "no adverse conditions", "rain", "sleet/hail/fog", "snow", 
    "other", "unknown"
  )
)
...
# convert TIME_cat to factor with levels "morning" "afternoon" "evening" and "night"
...
```{r print summary of relevant variables, include=FALSE}
...
```{r listwise models, eval=FALSE, include=FALSE}
# univariate model
...
```{r interaction models, eval=FALSE, include=FALSE}
multivariate_model_listwise_final_interaction <- model_eval_fun(
  formula = 
    "HIPR ~ HIC15_log + BMI + DV_MPH + BMI * HIC15_log + DV_MPH * HIC15_log",
  data = madymo_nhtsa_df_listwise
)
summary(multivariate_model_listwise_final_interaction$model)

# neither interaction term is significant, so univariate model is sufficient
```

```{r save models as objects, eval=FALSE, include=FALSE}
# save all models as objects to load later
save(
...
# save interaction_roc_grid_obj
save(
  interaction_roc_grid_obj,
  file = file.path("data", "interaction_roc_grid_obj.RData"
  ))
```

```{r load interaction grid object, echo=FALSE}
load(
  file = file.path("data", "interaction_roc_grid_obj.RData"
  ))
```
